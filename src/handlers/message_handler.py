...
def get_tts_engine(guild_id: int, config: Config):
    ...
    if guild_id not in tts_engines:
        engine_type = config.get_engine(guild_id)
        ...
        if engine_type == "gctts":
            ...
            tts_engines[guild_id] = EdgeTTSEngine(voice=voice)
    
    return tts_engines[guild_id]


async def generate_tts(tts_engine, text: str) -> Optional[str]:
